...
describe('v0.5 — contratos arquiteturais', () => {
  it('mantém a ordem de scripts e a paridade do helper', () => {
    ...
    const expected = ['storage', 'cloud', 'i18n', 'ships', 'achievements', 'audio', 'themes', 'events', 'input', 'powerups', 'missions', 'protocol', 'ghost', 'game', 'ui', 'share', 'main'];
    expect(scripts).toEqual(expected);
    const helper = read('tests/helpers/loadApp.js');
    expect(helper).toContain("['storage', 'cloud', 'i18n', 'ships', 'achievements', 'audio', 'themes', 'events', 'input', 'powerups', 'missions', 'protocol', 'ghost', 'game', 'ui', 'share']");
  });
  ...
  it('mantém versões coerentes para a release', () => {
    ...
    expect(read('src/sw.js')).toContain("const VERSION = '0.10.0'");
    expect(read('src/js/i18n.js')).toContain('v0.10.0');
  });
